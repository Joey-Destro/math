...
function showScreen(screenId) {
  ...
  else if (screenId === "build-screen")
    els.buildScreen.classList.remove("hidden");
  else if (screenId === "defend-screen")
    els.defendScreen.classList.remove("hidden");
  ...
  else if (screenId === "stats-menu-screen")
    els.statsMenuScreen.classList.remove("hidden");
...
function showComicText(text, x, y, color = "#fca311") {
  const pop = document.createElement("div");
  pop.classList.add("comic-text");
  pop.textContent = text;
  pop.style.left = `${x}px`;
  pop.style.top = `${y}px`;
  pop.style.color = color;
  els.enemiesArea.appendChild(pop);
...
function applyDynamicWeather() {
  els.gameContainer.classList.remove(
    "weather-rain",
    "weather-snow",
    "weather-night",
  );
  ...
  if (rand < 0.2) {
    els.gameContainer.classList.add("weather-rain");
  } else if (rand < 0.4) {
    els.gameContainer.classList.add("weather-snow");
  } else if (rand < 0.6) {
    els.gameContainer.classList.add("weather-night");
  }
}
...
function spawnEnemy(difficulty, isBoss = false) {
  ...
  const speed =
    (Math.random() * 0.5 + 0.5 + state.stats.wave * 0.1) *
    state.upgrades.slowMultiplier; // Rychlost podle vlny a upgrady
...
function updateDefendInputMode() {
  ...
  let closestEnemy = aliveEnemies.reduce((prev, curr) =>
...
function handleDefendInput(val) {
  ...
  if (hit && hitEnemyIndex !== -1) {
    ...
    shootProjectile(enemy, () => {
      ...
      let damage = 10 + state.upgrades.dmgBonus; // Zohlednění bonusového damage
      let killed = true;
      ...
      if (enemy.isBoss) {
        ...
        if (enemy.hp > 0) {
          killed = false;
          // Update boss HP bar
          const hpBar = enemy.el.querySelector(".boss-hp-bar");
          if (hpBar) {
            hpBar.style.width = `${(enemy.hp / enemy.maxHp) * 100}%`;
          }

          // Generate a new math problem for the boss so the player has to answer again
          enemy.problem = generateMathProblem(4, state.phase === "training");
          enemy.el.querySelector(".enemy-math").textContent =
            enemy.problem.text;

          // Shake effect
          enemy.el.style.animation = "shake 0.3s";
          setTimeout(
            () => (enemy.el.style.animation = "bounce 1s infinite alternate"),
            300,
          );

          enemy.dying = false; // reset dying state so player can target again
          els.defendInput.value = "";
        }
      }
      ...
      if (killed) {
        ...
        // Comic book POPUP text
        showComicText("BAM!", enemy.x, enemy.y);
        ...
        if (realIdx !== -1) {
          ...
          state.stats.kills++;
          let scoreEarned = 10 * (1 + Math.floor(state.stats.streak / 5)); // Bonus za streak
          if (enemy.isBoss) {
            scoreEarned += 100; // Velký bonus za zabití bosse
          }
          ...
          if (state.phase !== "training") {
            ...
            // Fire combo effects if streak is high
            if (state.stats.streak > 0 && state.stats.streak % 5 === 0) {
              showComicText(
                `${state.stats.streak}x COMBO!`,
                window.innerWidth / 2,
                100,
                "#e94560",
              );
              els.defendInput.classList.add("input-combo-fire");
              setTimeout(
                () => els.defendInput.classList.remove("input-combo-fire"),
                1000,
              );
            }

            // Každých 5 ve streaku = bonusová mince (s loot multiplierem)
            let baseCoins = 1 + Math.floor(state.stats.streak / 5);
            let coinsEarned = Math.floor(
              baseCoins * state.upgrades.lootMultiplier,
            );
            if (enemy.isBoss)
              coinsEarned += 10 * Math.floor(state.upgrades.lootMultiplier); // Loot za bosse
...
function endWave() {
  ...
  state.stats.wave++;
  if (state.stats.wave > state.stats.maxWave)
    state.stats.maxWave = state.stats.wave;
  saveGlobalStats();
